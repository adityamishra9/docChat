...
r.post("/upload", ensureAuthed, upload.single("pdf"), async (req, res) => {
  try {
    ...
    await streamFileToGridFS(req.file.path, req.file.originalname, req.file.mimetype, gridId);
    fs.unlink(req.file.path, () => {});

    const _id = new ObjectId();
    const collection = `pdf_${String(_id)}`;
    await col.insertOne({
      _id, ownerId: userId, name: req.file.originalname, size: req.file.size,
      status: "queued", pages: null, createdAt: new Date(), gridId, collection,
    });
